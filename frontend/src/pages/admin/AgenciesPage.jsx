...
function AgencyExpandCard({ agency, data, responseTimeTrend }) {
  ...
  const style = AGENCY_COLORS[agency.key] || { color: '#6B7280', bg: '#F3F4F6' };
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <div
        className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-[#263147] transition-colors"
        onClick={() => setExpanded((v) => !v)}
      >
        <div className="flex items-center gap-3">
          <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: style.color }} />
          <div>
            <p className="text-sm font-semibold text-[#F1F5F9]">{agency.label}</p>
            <p className="text-xs text-[#94A3B8]">{agency.vehicleType}</p>
          </div>
        </div>
        <div className="flex items-center gap-6">
          <div className="text-center">
            <p className="text-lg font-semibold text-[#F1F5F9]">{data?.activeIncidents ?? '—'}</p>
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-[#94A3B8]">Avg resp.</p>
          </div>
          <svg
      ...
      {/* Expanded content */}
      {expanded && (
        <div className="border-t border-[#334155] px-5 py-5 grid grid-cols-3 gap-6">
          {/* Severity breakdown */}
          <div>
            <p className="text-xs font-medium uppercase tracking-widest text-[#94A3B8] mb-3">
          ...
          <div>
            ...
            <div className="space-y-3">
              <VehicleBar label="Available" value={data?.vehiclesAvailable ?? 0} total={data?.totalVehicles ?? 0} color="#16A34A" />
              <VehicleBar label="Deployed" value={data?.vehiclesDeployed ?? 0} total={data?.totalVehicles ?? 0} color={style.color} />
              <VehicleBar label="Maintenance" value={data?.vehiclesMaintenance ?? 0} total={data?.totalVehicles ?? 0} color="#D97706" />
            </div>
          </div>
...
export function AgenciesPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-xl font-semibold text-[#F1F5F9] mb-5">Agency Overview</h1>
